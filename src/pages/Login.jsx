...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // console.log(obj)
    let findEmail = arr.find((user) => user.email === obj.email);
    let findPassword = arr.find((user) => user.password === obj.password);
    if (findEmail) {
      if (findPassword) {
        userCtx.setUserData({ login: true, email: findPassword.email });
        localStorage.setItem(
          "userLogin",
          JSON.stringify({ login: true, email: findPassword.email })
        );
        navigate("/");
...
<!-- Page Container --> */}
      <div
        id="page-container"
        className="login mx-auto flex min-h-dvh w-full min-w-[320px] flex-col bg-gray-100 dark:bg-gray-900 dark:text-gray-100"
      >
        {/* Page Content */}
        <main id="page-content" className="flex max-w-full flex-auto flex-col">
          <div className="relative mx-auto flex min-h-dvh w-full max-w-10xl items-center justify-evenly overflow-hidden p-4 lg:p-8">
            {/* Sign In Section */}
            <section className="w-full max-w-xl py-6">
              ...
              <header className="mb-10 text-center">
                <h1 className="mb-2 inline-flex items-center gap-2 text-2xl font-bold">
                  <img
                    ...
                    alt=""
                  />
                  <span className="text-black text-4xl underline">DAYA</span>
                </h1>
              </header>
              {/* END Header */}
              {/* Sign In Form */}
                  ...
                  <form className="space-y-6" onSubmit={handleSubmit}>
                    <div className="space-y-1">
                      <div className="flex justify-center items-center flex-col mb-12">
                        <h2 className="text-4xl font-bold text-black mb-2 text-center dark:text-gray-400">
                          Login
                        </h2>
                        <div className="w-[130px] h-1 border-b-4 border-blue-600 rounded-xl"></div>
                      </div>
              ...
              {/* END Sign In Form */}
              {/* Footer */}
              <div className="mt-6 text-center text-md md:text-xl text-white dark:text-gray-400">
                Powered by_
                <a
                  href="javascript:void(0)"
